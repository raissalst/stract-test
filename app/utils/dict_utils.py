...
def append_platform_and_user_info_to_insight_list(
    insight_list, platform_name, user_name, user_token
):
...
def replace_keys_with_correspondence(data_list, correspondences):
    ...
    transformed_data = [
        {mapping.get(key, key): value for key, value in entry.items()}
