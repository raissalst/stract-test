...
def fetch_accounts(platform_value):
    ...
    while True:
        conn.request("GET", f"{path}/api/accounts?platform={platform_value}&page={account_page}", headers=headers)
        response = conn.getresponse()
        
        if response.status != 200:
            # return {"error": "Failed to fetch accounts data", "status_code": response.status}
...
def fetch_fields(platform_value):
    ...
    while True:
        conn.request("GET", f"{path}/api/fields?platform={platform_value}&page={field_page}", headers=headers)
        response = conn.getresponse()

        if response.status != 200:
            # return {"error": "Failed to fetch field data", "status_code": response.status}
...
def fetch_insights(platform_value, platform_name, accounts, field_param):
    ...
    for account in accounts:
        conn.request("GET", f"{path}/api/insights?platform={platform_value}&account={account['id']}&token={account['token']}&fields={field_param}", headers=headers)
        response = conn.getresponse()

        if response.status != 200:
            # return {"error": "Failed to fetch insights data", "status_code": response.status}
        ...
        if not response_data:
            # return None, {"error": "Insights not found", "status_code": HTTPStatus.NOT_FOUND}
            raise ServiceError("Insights not found", HTTPStatus.NOT_FOUND)


        append_platform_and_user_info_to_insight_list(response_data, platform_name, account["name"], account["token"])
